...
public class OxygenProcessor {
    ...
    public OxygenProcessor(List<String> lines) {
        numbers = lines.stream().map(Long::parseLong).collect(Collectors.toList());

    }

    public OxygenProcessor(List<Long> numbers, int rel, int current) {
        this.current = current;
        this.numbers = new ArrayList<>(numbers);
        this.rel = rel;
    }

    public OxygenProcessor cloneProcessor() {

        return new OxygenProcessor(numbers, rel, current);
    }
        ...
        while (true) {
            ...
            switch ((int) action) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    int output = (int) getValue(numbers, param1, (current + 1));
                    current += 2;
